...
async function writeImportedEntry(input: ImportedEntryInput): Promise<'created' | 'updated'> {
    ...
    let existing = entryId === null
        ...
        : await admin
            .from('time_logs')
            .select('id, source, import_status, client_id, task_id, user_id, activity_keys, description, import_fingerprint, basecamp_entry_id')
    ...
    const row = mergeImportedEntry(
        existing.data
            ? {
                ...
                activityKeys: existing.data.activity_keys ?? [],
