...
create table public.content_opportunities (
  ...
  opportunity_type text not null check (opportunity_type in ('landing_page', 'supporting_article', 'location_page', 'existing_page_refresh', 'faq_addition', 'comparison_case_study', 'consolidate_redirect', 'no_action')),
  keyword text,
...
create policy "Org members can manage client intelligence" on public.client_intelligence for all using (organization_id in (select get_user_org_ids())) with check (organization_id in (select get_user_org_ids()));
create policy "Org members can manage content plans" on public.content_plans for all using (organization_id in (select get_user_org_ids())) with check (organization_id in (select get_user_org_ids()));
create policy "Org members can manage topic clusters" on public.topic_clusters for all using (organization_id in (select get_user_org_ids())) with check (organization_id in (select get_user_org_ids()));
create policy "Org members can manage content opportunities" on public.content_opportunities for all using (organization_id in (select get_user_org_ids())) with check (organization_id in (select get_user_org_ids()));
