...
- **Brand Theming** (migration 039, Aug 2026):
  ...
  - **No-flash is server-side.** `app/(dashboard)/layout.tsx` is a thin **server** layout that renders `<BrandThemeStyle />` (resolves the theme via `lib/theme/server.ts`) above the client `DashboardShell`. The colour ships with the first byte of HTML, so even a teammate who has never loaded the app never sees the default brand. A localStorage/cookie cache cannot do this — it is empty on the first load, which was exactly the case that flashed
  - Scoped to the dashboard segment on purpose: `cookies()` in the **root** layout would make the public marketing pages dynamic too. Dashboard routes are auth-gated and were never usefully static. Verified: `/`, `/login`, `/signup`, `/tools/*`, `/vs/*` stay `○ Static`; the dashboard segment is `ƒ Dynamic`
  - `selectedOrgId` is mirrored from localStorage into a **cookie** by `OrganizationProvider` so SSR picks the right org for multi-org users. With no cookie both sides fall back to the first membership — which is why **both** the provider query and the server resolver `.order('created_at')`. Drop that ordering and SSR can brand a different org than the client, reintroducing the flash
  - One `<style id="brand-theme">` element: server-rendered (`suppressHydrationWarning`), then mutated imperatively by `BrandThemeSync` / `previewThemeCss` for org switches and the Appearance preview. Verified exactly 1 element through a cold load and a live preview
  - The cookie carries **no authority** — the server still resolves membership through RLS
